...
const CreateAccountPage: React.FC = () => {
  ...
  const handleCreateAccount = async () => {
    ...
    for (const key in parameterDict) {
      const value = parameterDict[key];
      if (!value) {
    ...
    try {
      const response = await backendAPI.post("/users/create", parameterDict, {
        headers: {
  ...
  return (
        <main
            className="flex flex-col items-center justify-center w-full flex-1 px-20 text-center bg-gray-100 min-h-screen">
