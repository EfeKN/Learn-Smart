...
export default function CreateChatModal(modalProps: CreateChatModalParameters) {
  ...
  const handleUploadSlides = async () => {
    ...
    try {
      const response = await backendAPI.post(`/chat/create?course_id=${course_id}&title=${title}`, formData, {
        headers: {
  ...
  const handleCreateChat = async () => {
    ...
    try {
      const response = await backendAPI.post(`/chat/create`, {
        title: title,
        course_id: course_id,
      }, {
        headers: {
      ...
      <div className="bg-white p-8 md:p-12 rounded-lg shadow-xl w-1/2">
        ...
        <div className="flex justify-between">
          <div className="mr-4 p-4 border rounded relative">
            ...
            <button
              className="w-full mb-4 px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md shadow-sm hover:bg-blue-700 relative"
            >
            ...
            {isFileSelected && (
              <LoadingButton handleClick={handleUploadSlides} type="submit" text="OK" loadingText="Creating chat..." />
            )}
          </div>
